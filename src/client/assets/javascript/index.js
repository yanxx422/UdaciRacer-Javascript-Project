...
async function handleCreateRace() {
	...
	try {
		...
		store = Object.assign(store, { race_id: parseInt(race.ID) - 1 });
...
async function runCountdown() {
	try {
		...
		return new Promise(resolve => {
			...
			const countdown = setInterval(countdown_callback, 1000)
		})
	} catch(error) {
...
async function handleAccelerate() {
	...
	try{
		...
		console.log("Error with handleAccelerate()::", err)
	}
}
